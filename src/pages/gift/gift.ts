...
import {GiftImage, GiftStore} from "../../providers/giftstore";
...
export class GiftPage {
  ...
  private giftDeleted: boolean;
  private giftSaved: boolean;
  ...
  ionViewDidLoad() {
    ...
    if (this.gift.imageUUIDs) {
      this.gift.imageUUIDs.forEach(uuid => {
        this._giftStore
            .retrieveImage(uuid)
            .then(giftImage => this.images.push(giftImage))
      })
    }

    // if (this._platform.is("core") || this._platform.is("mobileweb")) {
    //   // TODO
    //   this.gift.title = "x" + Date.now();
  ...
  ionViewCanLeave(): boolean | Promise<void> {
    if (this.giftDeleted || this.giftSaved) {
      return true
    }
  ...
  save(gift: Gift, images: GiftImage[]) {
    if (gift.title.length === 0) {
      this._noticeCtrl.notice("Titel darf nicht leer bleiben");
    } else {
      this._giftStore
          .addOrUpdate(gift, images)
          .then(() => {
            this._noticeCtrl.notice("Geschenk gespeichert");
            this.giftSaved = true;
            this._nav.pop();
          })
          .catch(e => this._noticeCtrl.notice("Fehler beim Speichern: " + e))
    }
  }
